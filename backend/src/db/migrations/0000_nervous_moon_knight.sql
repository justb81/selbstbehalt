CREATE TABLE `bre_periods` (
	...
	`projected_bre` real,
	FOREIGN KEY (`insured_person_id`) REFERENCES `insured_persons`(`id`) ON UPDATE no action ON DELETE cascade
);
...
CREATE TABLE `contracts` (
	...
	FOREIGN KEY (`policyholder_id`) REFERENCES `persons`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
