...
//
// PWA acceptance checks (docs/architecture.md §8.6, issue #170): the app is
// installable (linked manifest with the required fields + icons), the service
// worker registers and precaches the shell, and the app shell loads offline.
...
test('exposes an installable web app manifest per §8.6', async ({ page }) => {
  ...
  // Unlike apps/frontend, GOÄ-Wächter has no reverse-proxy Basic Auth in front
  // of it (it is a public demo), so the manifest link needs no `crossorigin`.
  ...
  expect(manifest.name).toBe('GOÄ-Wächter');
...
test('serves robots.txt allowing crawling (public demo, issue #170)', async ({ request }) => {
