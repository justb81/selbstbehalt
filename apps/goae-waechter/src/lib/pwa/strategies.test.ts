...
const ctx = { selfOrigin: SELF, modelPathPrefix: '/models/' };
...
describe('classifyRequest', () => {
  ...
  it('leaves cross-origin requests untouched (no backend, no CDN)', () => {
    expect(classifyRequest('https://cdn.example/avatar.png', 'GET', ctx)).toBeNull();
  });

  it('ignores writes and non-http schemes', () => {
    expect(classifyRequest(`${SELF}/models/det.onnx`, 'POST', ctx)).toBeNull();
...
describe('cacheFirst', () => {
  ...
  it('does not cache a 206 Partial Content response and still returns it', async () => {
    ...
    const req = new Request(`${SELF}/_app/immutable/assets/chunk.abc123.mjs`);
    const res = await cacheFirst(req, deps(cache, fetchImpl));
